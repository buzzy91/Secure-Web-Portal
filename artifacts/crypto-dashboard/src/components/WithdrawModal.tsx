...
import { ArrowLeft, X, ChevronRight, AlertCircle, CheckCircle } from "lucide-react";
...
export default function WithdrawModal({ onClose }: Props) {
  ...
  function handleConfirm() {
    setPendingAmount(`$${parseFloat(amount).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={step === "pending" ? undefined : onClose} />
              ...
              <>
                <div className="mb-4">
                  ...
                  <input
                    type="text"
                    value={chimeContact}
                    onChange={(e) => setChimeContact(e.target.value)}
                    placeholder="Phone number or email on file"
          ...
          <div className="p-6 pb-10 flex flex-col items-center">
            ...
            <div className="w-full bg-[#0d1117] border border-[#1e2530] rounded-2xl divide-y divide-[#1e2530]">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <span className="text-white text-xs font-mono">WD-{Math.random().toString(36).slice(2, 10).toUpperCase()}</span>
              </div>
            </div>

            <p className="text-gray-600 text-xs text-center mt-5 leading-relaxed">
              You'll be notified once the withdrawal is approved and sent. Do not close this account during review.
            </p>
          </div>
        )}
      </div>
